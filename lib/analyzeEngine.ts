...
【輸出 schema】
{
  ...
  "items": [
    {
      "name": "給付項目名稱",
      "valueSource": "plan | table | insured | reimbursement | fixed（見下方判斷位階）",
      ...
      "insuredRate": { "type": "multiplier|percentage", "rate": 單一倍率, "min": 範圍最低, "max": 範圍最高 },  // valueSource=insured 或 reimbursement(限額×倍率) 皆填此；單一值用 rate，範圍用 min/max
...
3. reimbursement（限額）：實支實付型，條款寫「以限額為限／投保限額 × N倍」（住院醫療雜費、病房費等）→ 填 insuredRate（限額×倍率；單一用 rate，範圍用 min/max）
4. insured：非實支型，條款明確「保額 × N倍 / N%」→ 填 insuredRate（單一用 rate，範圍用 min/max）
5. fixed：直接固定金額（如住院日額 1000 元、每次最高 500 元）→ 填 amount
※ 位階重點：先看有無計劃/附表，有就用 plan/table；實支實付限額型用 reimbursement；非實支又講保額才用 insured；都不是才 fixed。
