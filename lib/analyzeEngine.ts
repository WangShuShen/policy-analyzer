...
【輸出 schema】
{
  ...
  "items": [
    {
      "name": "給付項目名稱",
      "valueSource": "plan | table | insured | unit | fixed | note（金額怎麼來，見下方判斷位階）",
      "isLimit": "true/false：給付性質是否為限額（實支實付上限型）。正交於 valueSource，只影響顯示",
      ...
      "insuredRate": { "type": "multiplier|percentage", "rate": 單一倍率, "min": 範圍最低, "max": 範圍最高 },  // 僅 valueSource=insured（保額×倍率）；單一值用 rate，範圍用 min/max
      "amount": 固定金額,  // valueSource=fixed（定額）或 unit（每壹單位金額）時填
...
1. plan：金額依計劃別（1~5 或 A/B/C）各自不同 → 填 planValues {計劃:金額}
2. unit：每壹單位給付（含「計劃1~N 等比放大」）→ valueSource=unit，amount 填每單位金額
3. table：金額查附表/單位別（如手術依項別）→ 填 tableRange 的最低~最高金額（不需逐項列）
4. insured：條款明確「保額 × N倍 / N%」→ 填 insuredRate（單一用 rate，範圍用 min/max）
5. fixed：直接固定金額（如住院日額 1000 元、每次最高 500 元）→ 填 amount
6. note（說明型）：給付依「所繳保費／加計利息／×健康促進係數／減已給付餘額」等，無法以保額/單位/計劃公式試算者 → valueSource=note，把計算說明寫進 formula（如「所繳保險費總和 × 1.3%」），不填 amount/insuredRate
※ 位階重點：先看有無計劃/單位/附表；再看是否講保額(insured)；金額能算出數字才用 fixed；只能依保費/餘額描述者用 note。
※ isLimit 另外獨立判斷：條款寫「以…為限／限額／實支實付上限」→ isLimit=true（與上面金額來源並存，例如 實支限額且各計劃不同 = plan + isLimit）。
...
- 實支實付：各項 isLimit=true（限額）；金額來源依條款判斷（常 plan 各計劃不同、或 fixed 固定上限、或 insured 保額×倍）
